...
SELECT
  ...
  (
    ...
    WHERE
      ...
      AND stats.action IN ('attack', 'block')
      AND stats.rating = 3 -- ask arnav what qualifies as a point
  ) / (
    SELECT
      COUNT(*)
    FROM
  ...
  (
    ...
    WHERE
      stats.player_id = players.id
      AND stats.action = 'set' -- ask arnav if there are other actions which constitute a "pass"
      AND stats.rating = 3
  ) / (
    SELECT
      COUNT(*)
  ...
  (
    ...
    WHERE
      stats.player_id = players.id
      AND stats.action IN ('attack', 'block') -- again, what constitutes a "point"?
  ) AS total_points,
  (
    SELECT
      COUNT(*)
  ...
  (
    ...
    WHERE
      stats.player_id = players.id
      AND stats.action IN ('serve_receive', 'freeball_recieve', 'set')
  ) AS total_def_points -- ?
FROM
  players;
...
-- Data
INSERT INTO
  players (id, first_name, last_name, grad_year)
VALUES
